...
!!! info "통신 방식"
    frontend는 ACA 내부 hostname `backend-service`로 backend를 호출합니다.
...
- replica 2개 상태에서 글을 작성해도 새로고침 시 데이터가 항상 유지될 것

!!! tip "힌트"
    Phase 3-2에서 배운 **Azure Files 볼륨**을 활용합니다.
    마운트 경로는 코드에 하드코딩되어 있습니다.
    경로가 맞지 않으면 앱이 죽습니다 — **로그 스트림**을 확인하세요.
...
| replica 2개 상태에서 새로고침해도 데이터 유지 확인 | ☐ |
