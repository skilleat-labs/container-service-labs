...
graph TB
  ...
  Internet["인터넷"] -->|"http://VM_IP:8000"| Web
  Web -->|"http://VM_IP:8080"| API
...
| API 접근 | `http://VM_IP:8080` (외부에서도 직접 접근 가능 — 보안 취약) |
...
!!! warning "AS-IS의 보안 문제"
    API가 포트 8080으로 외부에 노출되어 있어 누구나 `http://VM_IP:8080/orders?userId=3030` 을 직접 호출할 수 있습니다. ACA 이관 시 **Internal Ingress**로 해결합니다.
