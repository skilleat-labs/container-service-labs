...
??? note "pain-1-deploy-hell.sh 스크립트 동작 설명"
    이 스크립트는 총 4단계로 동작합니다.

    | 단계 | 내용 |
    |------|------|
    | 1단계 | v1 API 서버 상태 확인 (`/version` 호출) — 연결 안 되면 즉시 종료 |
    | 2단계 | 백그라운드에서 `/health` 를 0.5초 간격으로 20초간 호출하며 HTTP 상태 기록 |
    | 3단계 | `docker compose up -d` 로 v2 배포 시작 → 컨테이너 재생성으로 서비스 중단 발생 |
    | 4단계 | 배포 완료 후 결과 분석 출력, v1으로 자동 롤백 |

    배포 중 모니터링 출력 예시:
    ```
    10:30:01  [OK ] HTTP 200
    10:30:02  [!!!] HTTP ERR  ← 서비스 중단 발생!
    10:30:03  [!!!] HTTP ERR
    10:30:04  [OK ] HTTP 200
    ```

    `ERR`은 HTTP 응답 자체가 없는 상태(연결 거부)로, 503보다 심각합니다.

!!! tip "핵심: 스크립트 실행과 동시에 브라우저를 새로고침하세요"
    스크립트를 Enter로 실행한 직후, 브라우저 창으로 전환해 **F5(새로고침)를 빠르게 반복**합니다.
    배포 중 약 2~3초간 서비스가 중단되므로 그 순간을 직접 목격할 수 있습니다.

배포 중 브라우저에서 이런 화면이 나타납니다:

```
이 사이트에 연결할 수 없음  /  ERR_CONNECTION_REFUSED
```
