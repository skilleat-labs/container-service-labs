...
## ACA는 어떻게 ACR에서 이미지를 가져오나? — Managed Identity

ACI Smoke Test에서는 ACR 관리 사용자(username/password)로 이미지를 pull했습니다.
ACA에서는 더 안전한 방식인 **Managed Identity**를 사용합니다.

```
관리 사용자 방식:  ACA → ACR  "username: admin / password: xxxx"  ← 비밀번호 유출 위험
Managed Identity:  ACA → ACR  "저 Azure의 이 리소스예요"  ← Azure가 자동 증명, 비밀번호 없음
```

Managed Identity는 Azure 리소스에게 **사원증**을 발급하는 개념입니다.
ACA 앱이 "나는 이 ACR에서 이미지를 pull할 수 있는 리소스"임을 Azure가 자동으로 증명해줍니다.

!!! info "ACR 관리 사용자 비활성화"
    ACI Smoke Test가 끝났으니 ACR → **액세스 키** → **관리 사용자**를 다시 **비활성화**하세요.
    ACA는 Managed Identity로 pull하므로 관리 사용자가 필요 없습니다.
