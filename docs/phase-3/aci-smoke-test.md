...
3. **네트워킹** 탭으로 이동합니다
    ...
    |------|-----|
    | 네트워킹 유형 | 공용 |
    | DNS 이름 레이블 | `hanbat-smoke-1234` (숫자 부분을 바꿔서 전역 유일하게) |
    | 포트 | `8080` / TCP |

    !!! warning "DNS 이름 레이블 입력 규칙"
        - **접두사만** 입력합니다. `.koreacentral.azurecontainer.io`는 Azure가 자동으로 붙여줍니다.
        - 소문자, 숫자, 하이픈(`-`)만 사용 가능
        - 첫 글자는 영문자, 마지막 글자는 영문자 또는 숫자 (하이픈으로 끝나면 안 됨)
        - 전 세계에서 유일해야 하므로 뒤에 본인만의 숫자를 붙이세요 (예: `hanbat-smoke-7823`)
